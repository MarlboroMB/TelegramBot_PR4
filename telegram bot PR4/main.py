...
@dp.message_handler(states=Form.firstname)
...
@dp.message_handler(states=Form.lastname)
async def process_name(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
...
@dp.message_handler(lambda message: message.text, states=Form.group)
async def process_name(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
...
@dp.message_handler(lambda message: message.text not in ["2017", "2018", "2019", "2020"], states=Form.startedu)
async def process_gender_invalid(message: types.Message):
    return await message.reply("Ответ не верный. Выберите нужный вариант на клавиатуре.")

@dp.message_handler(state=Form.srartedu)
async def process_gender(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
        ...
        await bot.send_message(
            message.chat.id,
            md.text(
                md.text('Рад познакомиться!,', md.bold(data['firsname'] + " " + data['lastname'])),
